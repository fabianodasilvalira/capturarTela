...
class Vimeo
{
    ...
    public function upload($file_path, array $params = array())
    {
        ...
        if (!is_file($file_path)) {
            throw new VimeoUploadException('Não foi possível localizar o arquivo para fazer upload.');
        }
        ...
        if ($attempt['status'] !== 200) {
            ...
            throw new VimeoUploadException('Incapaz de iniciar um upload *.' . $attempt_error);
        }
    ...
    public function uploadImage($pictures_uri, $file_path, $activate = false)
    {
        ...
        if ($pictures_response['status'] !== 201) {
            throw new VimeoUploadException('Incapaz de solicitar um url de upload do vimeo');
        }
